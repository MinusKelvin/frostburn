...
pub(super) fn infer(stm: &[i16; HL_SIZE], nstm: &[i16; HL_SIZE], bucket: usize) -> i32 {
    ...
    *right = crelu(nstm);
    ...
    let mut result = NETWORK.l1.bias[bucket] as i32;

    for i in 0..activated.len() {
        result += activated[i] as i32 * activated[i] as i32 * NETWORK.l1.w[bucket][i] as i32;
    }
